...
temp_vars <- c("tmn_meanannual",
               "tmx_meanannual")
run_IDs <- c("vertica1.team.sdsc.edu_20141210230655",
             "vertica1.team.sdsc.edu_20141216061144")
...
foreach (temp_var=temp_vars, run_ID=run_IDs) %do% {
    ...
    ranefs_mu_B_g_names <- c("mu_B_g[2]",
                             "mu_B_g[3]",
                             "mu_B_g[4]",
                             "mu_B_g[5]",
                             "mu_B_g[6]",
    ...
    ranefs_mu_B_g_names_pretty <- c(expression(mu[P,g]),
                                    expression(mu[P^2,g]),
                                    expression(mu[T,g]),
                                    expression(mu[T^2,g]),
                                    expression(mu[D,g]),
    ...
    ranef_mu_b_g_scaling <- c((dbh_sd/precip_sd) * 100, # Convert from mm to 10s of cm
