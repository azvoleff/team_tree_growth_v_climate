...
seed <- 1638
stan_fit_test <- stan(model_file, data=model_data, iter=10,
                                    chains=1, init=rep(init_data, 1),
                                    refresh=1, seed=seed)
print("finished running test stan model")
save(stan_fit_test, file="stan_fit_simple_test.RData")

# Fit initial model, on a single CPU. Run only one iteration.
stan_fit_initial <- stan(model_file, data=model_data, iter=1,
                                       chains=1, init=init_data, chain_id=1)
print("finished running initial stan model")
save(stan_fit_initial, file="stan_fit_simple_initial.RData")
...
save(stan_fit_p, file="stan_fit_simple_parallel.RData")
